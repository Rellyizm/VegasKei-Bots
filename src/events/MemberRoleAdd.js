...
module.exports = async(member, role) => {
  ...
  if(!GeneralData) {let newGeneralDatabase = new GeneralDatabase({guildID: member.guild.id, userID: member.id, rollogtotal: 1, rollog: [{role: `${cfg.Emoji.TickEmoji} Rol: ${role} Yetkili: ${entry.executor}\nTarih: ${moment(Date.now()).locale("TR").format("LLL")}`}]}).save();} else{
  GeneralData.rollog.push({role: `${cfg.Emoji.TickEmoji} Rol: ${role} Yetkili: ${entry.executor}\nTarih: ${moment(Date.now()).locale("TR").format("LLL")}`}); 
  await GeneralDatabase.findOneAndUpdate({ guildID: member.guild.id, userID: member.id}, { $inc: { rollogtotal: 1 } }, { upsert: true })
  GeneralData.save();}
  member.guild.channels.cache.get("966507141584273428").send(new MessageEmbed().addField("Ekleyen Kişi", `${entry.executor} (\`${entry.executor.tag}\` - \`${entry.executor.id}\`)`, false).addField("Eklenen Rol", `${role} (\`${role.name}\` - \`${role.id}\`)`, false).addField("Eklenen Tarih", `${moment(Date.now()).locale("TR").format("LLL")}`, false).setDescription(`${member} (\`${member.user.tag}\` - \`${member.id}\`) kişisine bir rol eklendi.`).setThumbnail(entry.executor.avatarURL({dynamic:true})).setColor(role.hexColor).setAuthor(entry.executor.tag, entry.executor.avatarURL({dynamic:true})))
  } catch(err) {{}}}
